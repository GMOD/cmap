...
Postgres triggers have been written to keep the data in the CMap database syncronized with chado.  
...
=item - CMAP_BASE_NAME: This needs to be changed to what "CMAP_BASE_NAME" is set to in the cmap_syncronize_chado.pl script.  It is initially "cmap".
...
The options are similar to cmap_syncronize_chado.pl except db_base_name.  This
is the base name used to name the entries in the "db" table.  It will be set to
"cmap" by default (which is fine if the value in cmap_syncronize_chado.pl
wasn't changed). Only if there are multiple CMap databases that need to be
connected will this value be changed. 

=head1 Future stuff

Maybe include the trigger installation in the cmap_syncronize_chado.pl script
