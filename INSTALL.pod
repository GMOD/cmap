...
If you are upgrading from 0.12, please read the "upgrade-12-to-13.pod"
document in the "upgrade" directory.
...
directory.  These individual config files should be edited with specific 
...
"cmap.conf/" after installation with the correct database connection information.  
...
source directory of the CMap application.)  Multiple specific .conf files
can be used.  "mysql1.conf" is provided as a default conf file but should
be edited to your liking.  Also, additional examples have been provided as
*.conf.o files.
