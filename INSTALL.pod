...
=item * Install libpng and libgd 2.x
...
=item * A database (MySQL, Oracle, Sybase, PostgreSQL, SQLite) 

=item * Perl (5.6.1 or higher)

=item * A web server, Apache is suggested
...
supports CGI, not just Apache (e.g., Microsoft IIS, Oracle iPlanet Web Server, etc.).
...
CentOS/RHEL machine, the command "$ sudo yum list all | less" will
give you a list of the packages.  In Ubuntu, the equivalent command is
"$ apt-cache search ."
...
have to install the gd-devel package as well (RHEL/CentOS: gd-devel,
Ubundu: libgd2-xpm-dev).

With any luck, you may already have it
...
you want, like libpng and libgif.  Again, you may already
...
"/usr/local/lib" for files that look like "libgd*,"
"libpng*," and "libgif*".  If you're using something like RHEL,
...
=item * libgd: http://libgd.bitbucket.org/
...
such as MySQL (at least version 5.0 required), you can skip ahead to
the next section.  If not and you're doing this on your own and are
not a trained DBA, I'd recommend using SQLite (http://www.sqlite.org/)
...
of anything from the database except joins (no
...
you get really stuck, then Oracle, the company which develops and
...
http://www.apache.org/ and follow the directions there.
...
If you have any problems using a web server other than
...
Here is a list of directories needed for the CMap install.  The
...
This is the base directory for the cmap htdocs/ files.

=item * CACHE

The location where CMap stores it's cache files.  Usually
"HTDOCS/tmp". If this is a symbolic link to a non-existent directory, the
directory will be created during CMap execution.
...
for you:
  ...
  # cd /usr/local/apache
  # mkdir passwd
  # ./bin/htdigest -c /usr/local/apache/passwd/cmap admin
