...
=item * Modify database information in "cmap.conf/*.conf"
...
B<Note:> If you have version 0.09 of CMap installed, please read
APPENDIX 1 for instructions on upgrading.
...
such as MySQL (version 4.x recommended, but 3.23 is OK), you can skip
ahead to the next section.  If not and you're doing this on your own
and are not a trained DBA, I'd recommend using MySQL
...
"foobar":
  ...
  $ mysql -uroot -p CMAP < sql/cmap.create.mysql
      or
  $ mysql -uroot -p CMAP < sql/cmap.create.expanded.mysql
...
!NOTE! If you use the expanded version, make sure to set the expanded_correspondence_lookup to 1 in your config file for this database.
...
The "cmap.conf" directory will be created in the "conf" directory and will
contain a global.conf to set the "template_dir" and "cache_dir" to match 
the "TEMPLATES" and "CACHE" arguments to "Build.PL," respectively.  
It will then be copied to the "CONF" location.  Additionally, individual
database config files such as mysql1.conf will be copied into the cmap.conf
directory.
...
Be sure to update your database specific config files located in "cmap.conf/" after installation with the correct database connection information.  
(If you forget the location of the configuration file and you installed 
via option #1, then you can look in the "cmap_install.conf" file in the 
source directory of the CMap application.  The specific config file you use
is up to you, some examples have been provided as *.conf.o files)  
The database section is at the top of the
...
Two files B<will not> be overwritten if they exist unless you give the
OK:
...
=head1 APPENDIX 1. UPGRADING FROM VERSION 0.09
...
=head2 CREATE NEW TABLES AND INDICES
...
indices:
 ...
 CREATE INDEX ev_feature_correspondence_id ON cmap_correspondence_evidence 
...
=head2 DROP TABLES AND FIELDS (OPTIONAL)
...
=head1 APPENDIX 2. OPERATING SYSTEM
...
=head1 APPENDIX 3. CONVENTIONS FOR BUILDING SOFTWARE ON UNIX
...
=head1 APPENDIX 4. STARTING APACHE ET AL. ON BOOT
