...
The script will make a best guess for where to place components.
There are several configuration options that can be set via arguments to Build.PL.
Execute "perl Build.PL -h" to read a brief help message on what they
are and how you can specify your choices.  
...
The CMap installation script will make a guess at where to install
components.  These can be modified during installation or by giving
arguments like "PREFIX=/usr/local/apache2".
