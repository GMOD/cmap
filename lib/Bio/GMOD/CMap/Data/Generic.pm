...
=head2 get_feature_correspondence()
...
Return an array containing all cmap_feature_correspondence rows
containing feature_id1, or the row containing feature_id1, feature_id2 (if
exists).
...
=cut
    ...
    push @identifiers, $feature_id1;
    $sql_str .= " feature_id1 = ? ";

    if ($feature_id2) {
        push @identifiers, $feature_id2;
        $sql_str .= " AND feature_id2 = ? ";
    }

    my $sth = $db->prepare_cached($sql_str); 
    $return_object = $db->selectall_arrayref( $sth, { Columns => {} }, @identifiers )
        or return $self->error("No record for correspondence ");
