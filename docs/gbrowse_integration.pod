...
If you are going to copy data from the GBrowse side of the db into the CMap side, you will also need to add a "cmap_feature_type_accession" value to each track that you want to be able to copy.  An example track would look like this:
    ...
    cmap_feature_type_accession = contig
...
gbrowse_default_map_feature_type_aid.
...
string.
 ...
 gbrowse_ftype read
 area_code <<EOF
 $code=sprintf("onMouseOver=\"window.status='View %s in GBrowse';return true\"",$feature->{'feature_type_aid'});
 $alt = $feature->{'feature_name'};
 $url=sprintf("/cgi-bin/gbrowse/cmap?name=%s:%s",$self->feature_type_data($feature->{'feature_type_aid'},'gbrowse_ftype'),$feature->{'feature_name'});
 EOF
 </feature_type>
...
($feature->{'feature_type_aid'}) and the key that your looking for
