...
Feature and Evidence Type Information>.
...
=head2 Arguments for Speed and Clarity
...
=item * feature_type_DEFAULT

This will define the how to display feature types that are not explicitly
defined in feature_type_*, overriding the config setting.  Setting the features
...
=head2 Menu Arguments

=over 4

=item * feature_type_* [ default: set in config ]

These define how to display each feature type.  The "*" is replaced by the
feature type's accession (example: feature_type_marker=2).  Setting the
...
=item * evidence_type_* [ default: set in config ]

These define how to handle each evidence type.  The "*" is replaced by the
evidence type's accession (example: feature_type_blast=2).  
...
displayed, that correspondence will be displayed.
 ...
 Values:
    ...
    1 :  Display all features of this type
