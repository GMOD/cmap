...
The age (in seconds) of an image file that an image has to be to be purged.  
...
The maximum number of area boxes that CMap will print before it drops them all.
This can save a browser from crashing.  No commas allowed.

=item * max_query_cache_size [ default: 26214400 ]

How large the size limit should be for the query cache in bytes.  The value 0
will cause the size to not be limited.  This is used in the cmap_reduce_cache_size.pl (L<REDUCING QUERY CACHE SIZE>) script
...
For an example (a completely non-sensical example), lets pretend that a
...
factor will be used and it could create inconsistancies if one direction was
different from the other.

Also, multiple conversions can be difined as in this (non-sensical) example.
...
It is appropriate to describe all relationships as CMap won't neccessarily
...
inconsistancies, please make sure to define all relationships (and define them
correctly).
...
Set to one if the query cache should not be used.  This is handy for 
...
type.  They must be definined in <attribute> tags.  Here is an example entry:
...
CMap allows you to change the url, the alternet message or specify javascript
...
Set to 1 isable the query caching feature.  This is good for speed testing or
debugging.
...
You can use the cmap_validate_config.pl (L<VALIDATE CONFIG FILES>) to test your individual config files (not global.conf yet) and make sure that they are valid.  This can save you a lot of headaches dealing with configuration problems.  See the secion on L<VALIDATE CONFIG FILES> for more information.
...
Important Note:  As it is written, the links are stored in the temparary
directory '/tmp'.  When this directory is wiped, the links go away.  In future
versions of CMap this will change, but for now, you are warned.
...
See the above description forprev_ref_map_set_aid and extrapolate.
