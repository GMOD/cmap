...
see the section L<CONFIGURING DATABASE CONNECTIONS> later for more
...
Please see the "admintut" for examples of each shape and examples of
how these settings will affect the rendering of maps.
...
another CMap database.  For example, a "feature" object has a
"feature type," so to export it completely, one needs to export all
the information about both the feature and it's type (including color,
shape, drawing priority, etc.).  Likewise, to export a "map set"
object, one must export the map set's type, species, maps, features
and all the other dependent data (esp. the arbitrary attributes and
xrefs) on those objects.  This code is functional but still marked
...
"Import CMap objects."  Database objects are only create on import at
...
=head2 CREATING CORRESPONDENCE EVIDENCE TYPES
...
=head1 cmap_admin.pl and cmap_matrix_compare.pl
...
option.  Except for the "database," "template_dir" and "cache_dir"
settings, there is a reasonable default, hardcoded value for every
configuration setting (so you could actually comment out all of the
"optional settings" in the file).
...
=head2 CONFIGURING DATABASE CONNECTIONS
...
=head2 CONFIGURING TEMPLATE DIRECTORY

The template directory will be properly set if you went through the
"perl Build.PL; ./Build; su; ./Build install" routine for installing
CMap.  You can define this with the "TEMPLATES" arguemnt to "Build.PL"
or just accept the default.  If you ever wish to change the location
of the templates, simply update "cmap.conf" with the new directory.
...
improvement, please relay them to the author, prefereably via the
